...
class MazeGenerator:
    ...
    def access_structure(self) -> list[list[dict[str, int]]]:
        """Access the generated structure in a readable way.

        Returns a list of list of dict where each list represents
        a row and each dict a cell with its 4 walls encoded clearly.
        """
        result = []
        generated = self.board.grid
        for line in generated:
            row = []
            for room in line:
                row.append(room.walls)
            result.append(row)
        return result
